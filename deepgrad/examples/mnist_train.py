...
def convert_csv_to_bin(csv_path, bin_path, normalize=True):
    MNIST_MEAN = 0.1307
    MNIST_STD = 0.3081
...
def build_batch_from_mmap_back(mm, sample_indices, input_size):
...
if __name__ == '__main__':
    ...
    for passes in trange(total_steps, desc="Training", ncols=100):
        ...
        x = Tensor(x_array, requires_grad=True, shape=(actual_bs, 1, 28, 28), size=actual_bs * INPUT_SIZE)
        y = Tensor(y_array, requires_grad=False, shape=(actual_bs,), size=actual_bs)
    ...
    print(f"Final Test Accuracy: {acc:.2f}, Model saved to mnnist_model.pkl")
