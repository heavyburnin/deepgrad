...
void col2im(const float* im2col_grad, float* grad_input,
    ...
    for (size_t n = 0; n < N; ++n) {
        for (size_t h_out = 0; h_out < H_out; ++h_out) {
            for (size_t w_out = 0; w_out < W_out; ++w_out) {
                size_t out_row = h_out * W_out + w_out;
                const float* src = im2col_grad + n * H_out * W_out * C * K_h * K_w + out_row * C * K_h * K_w;
                ...
                for (size_t c = 0; c < C; ++c) {
                    for (size_t kh = 0; kh < K_h; ++kh) {
                        for (size_t kw = 0; kw < K_w; ++kw) {
                            ...
                            if (h_in >= 0 && h_in < H && w_in >= 0 && w_in < W) {
                                #pragma omp atomic
                                grad_input[((n * C + c) * H + h_in) * W + w_in] += src[patch_idx];
                            }
                            patch_idx++;
                        }
                    }
...
void conv2d_forward_gemm(const float* input, const float* weight, const float* bias, float* output,
    ...
    float* im2col_buf = aligned_malloc(N * M * K);
    float* output_buf = output;

    #pragma omp parallel for
    for (size_t n = 0; n < N; ++n) {
        const float* img = input + n * C_in * H_in * W_in;
        float* dst = im2col_buf + n * M * K;
        im2col_single(img, dst, C_in, H_in, W_in, K_h, K_w, pad_h, pad_w, stride_h, stride_w, H_out, W_out);
    }

    tensor_matmul(MATMUL_FORWARD, im2col_buf, weight, NULL, output_buf, NULL, N, M, K, C_out, false);
...
void conv2d_backward_gemm(const float* input, const float* weight, const float* grad_out,
    ...
    float* im2col_buf = aligned_malloc(N * M * K);
    float* grad_im2col = aligned_malloc(N * M * K);
    ...
    tensor_matmul(MATMUL_FORWARD, im2col_buf, grad_out, NULL, grad_weight, NULL, N, K, M, C_out, true);
    tensor_matmul(MATMUL_FORWARD, grad_out, weight, NULL, grad_im2col, NULL, N, M, C_out, K, false);

    #pragma omp parallel for
    for (size_t n = 0; n < N; ++n) {
        float* dst = grad_input + n * C_in * H_in * W_in;
        const float* src = grad_im2col + n * M * K;
        col2im(src, dst, 1, C_in, H_in, W_in, K_h, K_w, pad_h, pad_w, stride_h, stride_w, H_out, W_out);
    }

    if (grad_bias) {
        #pragma omp parallel for reduction(+:grad_bias[:C_out])
        for (size_t n = 0; n < N; ++n) {
            for (size_t h = 0; h < H_out; ++h) {
                for (size_t w = 0; w < W_out; ++w) {
                    size_t offset = (n * H_out + h) * W_out + w;
                    for (size_t c = 0; c < C_out; ++c) {
                        grad_bias[c] += grad_out[offset * C_out + c];
                    }
                }
            }
        }
